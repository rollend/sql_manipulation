...
def find_duplicates(database,sync,table):
    host,db,user,passwd,db,charset=connect_database(database)
    con=pymysql.connect(host=host,user=user,passwd=passwd,db=db,charset=charset)
    ...
    if sync=='Full':    
        ...
        if conti_not == 'Yes':              
            ...
            for i in sync_inf['sync_id']:
                ...
                p_clean, p_duplicate, duplicates_parameters= find_duplicates(database,i,table)
                p_duplicate_validate = validating(p_clean, p_duplicate, duplicates_parameters,table)        
                commit_to_database(p_duplicate_validate,database,i,table)
                sucess_list[i]=len(p_duplicate)
        ...
        if conti_not == 'Yes':            
            ...
            records_sync=pd.DataFrame() 
            p_clean, p_duplicate, duplicates_parameters= find_duplicates(database,sync,table)
            p_duplicate_validate = validating(p_clean, p_duplicate, duplicates_parameters,table)        
            commit_to_org_database(p_duplicate_validate,database,sync,table)
            ...
            p_clean, p_duplicate, duplicates_parameters= find_duplicates(database,sync,table)
            p_duplicate_validate = validating(p_clean, p_duplicate, duplicates_parameters,table)        
            commit_to_database(p_duplicate_validate,database,sync,table)
            sucess_list[sync]=len(p_duplicate)
            instances=pd.read_sql("SELECT count(*) AS SUM FROM %s" %table,con)
            instances_new=pd.read_sql("SELECT count(*) AS SUM FROM %s_new" %table,con)
            assert(sum(sucess_list.values())==(instances['SUM'][0]-instances_new['SUM'][0]))
